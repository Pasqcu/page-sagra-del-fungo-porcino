...
Obiettivo: **https://www.sagradelfungoporcino.com/Ordini/**
...
- il pacchetto `Ordini.zip` preparato in questa cartella
...
Scompatta `Ordini.zip` sul tuo computer: ottieni una cartella `Ordini`.

Poi, in FileZilla, trascina l'intera cartella `Ordini` dal riquadro di sinistra (il tuo
...
puoi caricare direttamente `Ordini.zip` nella cartella pubblica e usare la funzione di
estrazione. Assicurati poi che il risultato sia `Ordini/index.html` e non `Ordini/Ordini/index.html`.

### La maiuscola conta

Il server è Linux: `Ordini` e `ordini` sono due indirizzi diversi. La cartella deve chiamarsi
esattamente **`Ordini`**, con la O maiuscola, perché l'indirizzo richiesto è `/Ordini/`.
Chi digiterà `/ordini/` minuscolo prenderà un 404.

---

## Passo 5 — Verifica

Apri **https://www.sagradelfungoporcino.com/Ordini/** dal telefono, non dal computer:
...
- Maiuscola sbagliata: la cartella deve essere `Ordini`, non `ordini`
- Doppia cartella: hai `Ordini/Ordini/index.html`. Sposta i file su di un livello
- Cartella caricata nel posto sbagliato: rileggi il passo 3

**Errore 500**
- Cancella il file `.htaccess` dentro `Ordini` e ricarica la pagina.
...
1. Entra in `wp-admin`
2. **Aspetto → Menu**
3. **Link personalizzato** → URL `https://www.sagradelfungoporcino.com/Ordini/`,
   testo `Ordina`
4. **Aggiungi al menu**, poi **Salva menu**

Non creare una pagina WordPress chiamata "Ordini": creerebbe confusione con la cartella.
